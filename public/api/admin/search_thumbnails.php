...
if (!is_string($query)) {
    respond(400, ['success' => false, 'message' => 'Invalid search query']);
}
...
try {
    $search = (new ThumbnailService())->searchPlaylist(trim($playlist), $query);
    if ($search === null) {
        respond(404, ['success' => false, 'message' => 'Playlist not found']);
    }

    respond(200, array_merge(['success' => true], $search));
} catch (\Throwable $e) {
    error_log('Search Thumbnails API Error: ' . $e->getMessage());
